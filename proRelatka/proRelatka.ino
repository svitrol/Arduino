...
bool mamTamHodnotu[PocetRelecek]={false,false,false,false,false,false,false,false};
...
void projdiSiPodminky(){
	for (byte i = 0; i< pocetPodminek; i++) {
		podminka(podminky[i]);
	}
}
...
void vykonajFejnovyPrikaz(char* prikaz){
	...
	const char* odkazy[4];
	odkazy[0]=prikaz;
	uint8_t pocetOdkazu=1;	
	for(uint8_t i=0;prikaz[i]!='\0';i++){
		if(prikaz[i]=':'){
			prikaz[i++]='\0';			
			odkazy[pocetOdkazu++]=&prikaz[i];
		}
	}
    if (prikaz[0] == 'R') {
	    switch (prikaz[1] - 48) {
		    ...
		    case 8: {
			    byte akce = 0;
			    if (prikaz[3] == '1') { //formát zprávy Rx:0/1:1/x/c
				    akce = 1;
				    //Serial.printf("ano zapnu ti ho %s",pole);
			    }
			    if (akce != relatka[prikaz[1] - 48 - 1]) {
				    relatka[prikaz[1] - 48 - 1] = akce;
				    uprava = 1;
				    poslirelecka = 1;
			    }
			    break;
		...
		for(uint8_t i=0;odkazy[1][i]!='\0';i++){
			if(odkazy[1][i]!='X'){
				relatka[i]=odkazy[1][i]-48;
			}			
		}
		uprava = 1;
		poslirelecka = 1;
